...
abstract class Oauth {
    ...
    public function __construct($token = null) {
        ...
        $this->Type = strtoupper(substr($class, strrpos($class,'\\')+1, - 3));
        //获取应用配置
        ...
        if (empty($config['APP_KEY']) || empty($config['APP_SECRET'])) {
            ...
            $this->Token = is_array($token) ? $token : array('access_token' => $token); //设置获取到的TOKEN
        }
    }
    ...
    abstract public function getUserInfo($api,$openid);//by zhouwei
    abstract public function convertUserInfo($userinfo);//by zhouwei
}