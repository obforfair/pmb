...
class WeixinSDK extends Oauth{
	...
	public function openid(){
        ...
        public function getUserInfo($api,$openid) {           
            ...
            $data = json_decode($this->http($this->url($api), $params),true);
            return $data['nickname'] ? $this->convertUserInfo($data) : false;
        }
        
        public function convertUserInfo($userinfo) {
            $data['nickname'] = $userinfo['nickname'];
            $data['gender'] = $userinfo['gender'];
            $data['avata'] = $userinfo['figureurl_qq_2'] ? $userinfo['figureurl_qq_2'] : $userinfo['figureurl_qq_1'];
            $data['gender'] = ($userinfo['gender'] == '男') ? 1 : 0;
            $data['gender'] = ($userinfo['gender'] == '女') ? 2 : $data['gender'];   
            return $data;
        }
}